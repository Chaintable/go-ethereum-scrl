...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *worker {
	...
	worker.asyncChecker = ccc.NewAsyncChecker(worker.chain, runtime.GOMAXPROCS(0)).WithOnFailingBlock(worker.onBlockFailingCCC)
...
func (w *worker) newWork(now time.Time, parentHash common.Hash, reorgReason error) error {
	...
	// Only set the co"inbase if our consensus engine is running (avoid spurious block rewards)
	if w.isRunning() {
	...
	cccLogger := ccc.NewLogger(1_000_000)
	vmConfig.Debug = true
...
func (w *worker) tryCommitNewWork(now time.Time, parent common.Hash, reorgReason error) (common.Hash, error) {
	...
	if !shouldCommit && w.chain.GetBlockByNumber(w.current.header.Number.Uint64()) != nil {
...
func (w *worker) processReorgedTxns(reason error) (bool, error) {
	...
	if errors.As(reason, &errorWithTxnIdx) {
		if errorWithTxnIdx.TxIdx == 0 {
			...
			w.skipTransaction(reorgedTxns[0], reason)
		} else {
